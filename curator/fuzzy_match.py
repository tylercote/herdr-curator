"""Fuzzy find-and-replace for LLM-generated edits (verbatim port of ``tools/fuzzy_match.py``).
...
def _calculate_line_positions(content_lines, start_line: int, end_line: int, content_length: int):
...
def _window_spans(content: str, content_lines, n: int, accept: Callable[[int], bool]):
    return [_calculate_line_positions(content_lines, i, i + n, len(content))
            for i in range(len(content_lines) - n + 1) if accept(i)]


def _match_transformed_lines(content: str, pattern: str, transform):
...
def _build_orig_to_norm_map(original: str):
    result = []
    norm_pos = 0
...
def _invert_norm_map(orig_to_norm):
    inverted = {}
...
def _norm_end_to_orig(orig_to_norm, orig_start: int, norm_end: int) -> int:
...
def _map_positions_norm_to_orig(orig_to_norm, norm_matches):
...
def _map_normalized_positions(original: str, normalized: str, normalized_matches):
    orig_to_norm = []
    orig_idx = norm_idx = 0
...
def _strategy_exact(content: str, pattern: str):
    return [m.span() for m in re.finditer(re.escape(pattern), content)]


def _strategy_line_trimmed(content: str, pattern: str):
    return _match_transformed_lines(content, pattern, lambda ls: [l.strip() for l in ls])


def _strategy_whitespace_normalized(content: str, pattern: str):
    def normalize(s):
        return re.sub(r'[ \t]+', ' ', s)
...
def _strategy_indentation_flexible(content: str, pattern: str):
    return _match_transformed_lines(content, pattern, lambda ls: [l.lstrip() for l in ls])


def _strategy_escape_normalized(content: str, pattern: str):
...
def _strategy_trimmed_boundary(content: str, pattern: str):
    return _match_transformed_lines(content, pattern, _strip_boundary)


def _strategy_unicode_normalized(content: str, pattern: str):
...
def _strategy_context_aware(content: str, pattern: str):
    ...
    def accept(i: int) -> bool:
        ...
        if _sim(last_pat, block_lines[-1].strip()) < 0.80:
            return False
        return all(not p_line.strip() or _sim(p_line.strip(), c_line.strip()) >= 0.80
...
def _matched_regions(content: str, matches) -> str:
    return "".join(content[start:end] for start, end in matches)


def _format_match_locations(content: str, matches, cap: int = 5) -> str:
    rows = []
...
def fuzzy_find_and_replace(content: str, old_string: str, new_string: str, replace_all: bool = False):
    """Returns ``(new_content, match_count, strategy_name, error)``; on failure ``(content, 0, None, error)``."""
    if not old_string:
    ...
    for strategy_name, strategy_fn in STRATEGIES:
        ...
        new_content = _apply_replacements(content, matches, effective_new,
...
def _detect_escape_drift(content: str, matches, old_string: str, new_string: str) -> Optional[str]:
...
def _backslash_runs(s: str):
    return [len(run) for run in re.findall(r"\\+", s)]


def _detect_backslash_doubling(matched_regions: str, old_string: str, new_string: str) -> Optional[str]:
...
def _maybe_unescape_new_string(new_string: str, content: str, matches) -> str:
...
def _preserve_unicode_in_replacement(content: str, matches, old_string: str, new_string: str) -> str:
...
def _apply_replacements(content: str, matches, new_string: str, old_string: Optional[str] = None) -> str:
    result = content
